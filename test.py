...
def NN(net, lemniscate, trainloader, testloader, recompute_memory=0):
    ...
    if hasattr(trainloader.dataset, 'imgs'):
        ...
        trainLabels = torch.LongTensor(trainloader.dataset.classes).cuda()
    ...
    if recompute_memory:
        ...
        trainLabels = torch.LongTensor(temploader.dataset.classes).cuda()
...
def kNN(net, lemniscate, trainloader, testloader, K, sigma): #, recompute_memory=0): # Changed to recompute_memory in main
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, indexes) in enumerate(tqdm.tqdm(testloader)):
            ...
            preds = np.concatenate((preds, predictions[:,0].cpu().data))
    ...
    table = pd.DataFrame({'classes': list(set(trainloader.dataset.classes)), #class_to_idx.keys()),
